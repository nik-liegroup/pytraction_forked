...
def read_roi_file(fpath):
    ...
    if version >= 227 and roi['type'] == 'rectangle':
        NW = [roi['left'], roi['top']]
        NE = [roi['left'] + roi['width'], roi['top']]
        SE = [roi['left'] + roi['width'], roi['top'] + roi['height']]
        SW = [roi['left'], roi['top'] + roi['height']]

        coordinates = np.array([NW, NE, SE, SW])

        roi.update(dict(x=coordinates[:,0], y=coordinates[:,1]))
