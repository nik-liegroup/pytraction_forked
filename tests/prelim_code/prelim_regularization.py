...
def log_evidence_ft(alpha, beta, C_a, BX_a, X, fuu, constant, elastic_modulus, s, xx, yy, inverse_ux, inverse_uy):
    aa = X.shape
    lambd = alpha / beta
    fx, fy, ft_fx, ft_fy, ft_ux, ft_uy, kxx, kyy, X = traction_fourier(
        xx, yy, inverse_ux, inverse_uy, s, elastic_modulus, lambd=lambd, scaling_factor=None, zdepth=0, slim=True
    )
...
def bayesian_regularization_ft(
    beta, ftux, ftuy, elastic_modulus, s, i_max, j_max, X, xx, yy, inverse_ux, inverse_uy
):
    ...
    target = partial(
        ...
        xx=xx,
        yy=yy,
        inverse_ux=inverse_ux,
        inverse_uy=inverse_uy
    )
